...
class Trainer:
    def __init__(self, env: GameEnv, agent: DQNAgent, 
        ...
        # self.scheduler = optim.lr_scheduler.StepLR(self.agent.optimizer, 1000, gamma=config.get("LR_GAMMA", 0.99))
        
        self.done = False
        
        self.max_episodes = config.get("MAX_EPISODES", 1000)
        
        self.update_frequency = config.get("UPDATE_FREQ", 50)
    ...
    def train(self):
        ...
        for episode in progress_bar:
            ...
            # Update progress bar
            progress_bar.set_postfix({"reward": f"{self.env.get_episode_reward():.2f}"})
            ...
            self.writer.add_scalar("Reward/Train", self.env.get_episode_reward(), episode)
